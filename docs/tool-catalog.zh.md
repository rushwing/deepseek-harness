...
驱动一个需求（REQ）走过生命周期：每一步生成一个由 agent 注册表落座的新鲜角色子代，把它的编辑围在自己的工件之内，对照生命周期表判定其迁移提案，应用效果并 lint；人类拥有的状态由人类决定。当 REQ 完成或被阻塞、等待人类决定、某步被拒或失败、树 lint 为红、或达到步数上限时停止；子代也可以用一个给人类的问题暂停运行。仅在被要求驱动 REQ 时使用。
...
手工对一个需求（REQ）应用一次生命周期迁移（如 T01）或生命周期事件（如 bug_fix）：检查守卫，效果重写 frontmatter 与状态，并对结果做 lint；红色结果不写任何东西。返回建议的提交主题；由人类提交。由人类决定的迁移（如 T01、T14）在此被拒绝：由人类运行 /lifecycle transition。
