...
| [conversation-backend.md](conversation-backend.zh.md) | 运行在外部 agent 产品上的会话后端：请求分类、Session 绑定、权限桥接与共享的失败分类 |
| [lifecycle-team.md](lifecycle-team.zh.md) | 生命周期团队：工作区文件、`ctx.lifecycle` 服务与其工具、三项硬停检查与失败码 |
