...
describe('the transition proposal', () => {
  it('declares a structured-output schema with the summary required', () => {
    expect(PROPOSAL_SCHEMA.required).toEqual(['summary'])
    expect(Object.keys(PROPOSAL_SCHEMA.properties ?? {})).toEqual(['transition', 'event', 'summary', 'decisions', 'pr'])
  })
  ...
  it('reads structured output first, then the last fenced JSON block of the text', () => {
    ...
    expect(parseProposal(structured)).toEqual({ ok: true, proposal: { transition: 'T02', event: undefined, summary: 'from schema', decisions: {}, pr: undefined } })
    ...
    expect(parseProposal(twoBlocks)).toEqual({
      ok: true,
      proposal: { transition: undefined, event: 'bug_fix', summary: 'last', decisions: { fields: { a: 1 }, tcStatuses: { 'TC-1': 'passing' } }, pr: 7 },
    })
