/**
 * The English role × state briefs `lifecycle_init` scaffolds to
 * `<lifecycleDir>/standards/briefs.md`, translated from the factory-tools
 * `briefs.md` with the hand-over rewritten for the orchestrator: a role proposes
 * a transition instead of committing one. `briefs/parse.ts` reads this format.
 *
 * @module @deepseek-ai/dsh-experimental-lifecycle-orchestrator/defaults/briefs
 */

/** The default `standards/briefs.md`. */
export const BRIEFS: string = `# Briefs (Role x State Task Briefs)
