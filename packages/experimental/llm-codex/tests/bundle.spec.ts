/** The Codex backend bundle must carry one parseable layer: the adapter row plus a Web-only preset row. */
...
import { Config } from '@deepseek-ai/dsh-experimental-llm-codex'

describe('Codex backend bundle', () => {
  it('declares a public parseable layer with the adapter row and a profile-gated preset row', () => {
    ...
    expect(inserted.map(entry => entry.id)).toEqual(['llm-codex', 'preset-codex'])
    const adapter = inserted[0]!
    expect(adapter.name).toBe('@deepseek-ai/dsh-experimental-llm-codex')
    expect(adapter.disabled).toBeUndefined()
    expect(Object.keys(new Config(adapter.config ?? {}).routes ?? {})).toEqual(['codex'])
