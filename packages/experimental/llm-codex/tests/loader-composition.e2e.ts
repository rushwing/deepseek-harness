...
const configPath = join(fixtureDir, 'llm-codex.patch.yml')
const packageDir = fileURLToPath(new URL('..', import.meta.url))
const manifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as {
  dsh?: { bundle?: { patch?: string } }
}
const bundlePatch = manifest.dsh?.bundle?.patch
if (bundlePatch === undefined) throw new Error('Codex backend package must declare a Bundle patch')
const bundlePatchPath = join(packageDir, bundlePatch)
const repoTsconfig = fileURLToPath(new URL('../../../../tsconfig.json', import.meta.url))

describe('Codex backend public Loader composition', () => {
  it('registers the configured routes in the headless profile without starting the product', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      label: 'llm-codex Loader composition',
      tempDirPrefix: 'dsh-llm-codex-loader-',
      binScript: driver,
      libBinScript: driver,
      configPath,
