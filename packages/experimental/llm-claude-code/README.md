...
The CLI appends this package's [`cordis.patch.yml`](cordis.patch.yml) as a profile layer: the adapter row registers the routes in every profile, and a `Claude Code` agent preset (one complete persona, no dsh tools or runtime context) joins the Web preset picker wherever the preset registry is mounted. Select the route as the Session's provider (`provider: claude-code`) with a model id or alias the CLI accepts (`sonnet`, `opus`, or a full model id); the Web model picker lists the catalog the CLI advertises. Remove the layer through the same CLI with `remove @deepseek-ai/dsh-experimental-llm-claude-code`.
