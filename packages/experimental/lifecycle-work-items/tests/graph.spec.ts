...
describe('loadGraph', () => {
  it('loads the translated factory-tools fixtures into a linked graph', () => {
    ...
    expect(graph.ownTcsOf('REQ-PLAT-009').map(tc => tc.id).slice(0, 2)).toEqual(['TC-PLAT-009-01', 'TC-PLAT-009-02'])
    expect(graph.carriedBugsOf('REQ-PLAT-009').map(bug => bug.id)).toEqual(['BUG-PLAT-003', 'BUG-PLAT-004', 'BUG-PLAT-005', 'BUG-PLAT-006'])
    expect(graph.blockingBugsOf('REQ-PLAT-009')).toEqual([])
    expect(graph.ownTcsOf('REQ-PLAT-010')).toEqual([])
